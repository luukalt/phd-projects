...
def read_piv_data(image_nr):
    ...
    pivot_velocity_abs = pd.pivot_table(df_piv, values=headers[4], index=headers[1], columns=headers[0])
    velocity_abs = pivot_velocity_abs.values
    ...
    return contour_nr, n_windows_x, n_windows_y, x/D, y/D, u/U_bulk, v/U_bulk, velocity_abs/U_bulk, divergence_u_davis
...
def determine_local_flame_speed(selected_segments_2, final_segments, colors, ax=None):
    ...
    # - final_segment = [selected_segment, x_intersect_ref, y_intersect_ref, x_intersect, y_intersect, V_n, S_a]
...
def local_flame_speed_from_double_image_single_frame(image_nr, n_nearest_coords=1, threshold_angle=180):
    ...
    for i, image_nr in enumerate(image_nrs):
        ...
        # Read PIV data
        contour_nr_t0, n_windows_x, n_windows_y, x_t0, y_t0, u_t0, v_t0, velocity_abs_t0, _ = read_piv_data(image_nrs[0])
        contour_nr_t1, n_windows_x, n_windows_y, x_t1, y_t1, u_t1, v_t1, velocity_abs_t1, _ = read_piv_data(image_nrs[1])
        ...
        selected_segments_1_t0, selected_coords = first_segment_selection_procedure(contour_corrected_t0, n_windows_x, n_windows_y, x_t0, y_t0, u_t0, v_t0, n_nearest_coords)
        ...
        if toggle_plot:
            ...
            if i == 0:
                ...
                for j, selected_coord in enumerate(selected_coords):
                    ...
                    (_, _, _, x_piv_ij, y_piv_ij, _, _, i_segment, x_mid, y_mid) = selected_coord
...
def local_flame_speed_from_time_resolved_single_frame(image_nr, n_nearest_coords=1, threshold_angle=180):
    ...
    # read PIV data
    contour_nr_t0, n_windows_x, n_windows_y, x_ref, y_ref, u_ref, v_ref, velocity_abs_ref, _ = read_piv_data(image_nr_t0)
    contour_nr_t1, n_windows_x, n_windows_y, x, y, u, v, velocity_abs, _ = read_piv_data(image_nr_t1)
...
def check_intersection_normal_vector_and_segment(p1, p2, p3, p4):
...
def first_segment_selection_procedure(contour_corrected, n_windows_x, n_windows_y, x, y, u, v, n_nearest_coords):
    ...
    for i_segment in range(0, len(contour_x) - 1):
        ...
        b1 = y_A - normal_slope * x_A
        b2 = y_B - normal_slope * x_B
        ...
        for j, i in np.ndindex((n_windows_y, n_windows_x)):
            ...
            if np.isfinite(divergence_u[j, i]) :
                ...
                # Calculate the perpendicular distance for each coordinate
                distance_to_normal_line_A = abs(y_piv_ij - (normal_slope * x_piv_ij + b1)) / np.sqrt(1 + normal_slope**2)
                distance_to_normal_line_B = abs(y_piv_ij - (normal_slope * x_piv_ij + b2)) / np.sqrt(1 + normal_slope**2)
            
                coord_u = [j, i, distance_to_segment_midpoint, x_piv_ij, y_piv_ij, u_ij, v_ij, i_segment, x_mid, y_mid]
                coords_u.append(coord_u)
                
                if distance_to_normal_line_A <= L and distance_to_normal_line_B <= L:
        ...
        if candidate_coords_u:
            ...
            x_loc, y_loc = selected_coord_u[3], selected_coord_u[4]
            ...
            if (y_A or y_B) >= y_top_raw:
                ...
                selected_segment_1 = [x_A, y_A, x_B, y_B, x_mid, y_mid, x_loc, y_loc, V_nx_select, V_ny_select, np.abs(V_n), V_tx_select, V_ty_select, V_t, V_x_select, V_y_select, i_segment, contour_corrected, rotation_matrix]
                selected_segments_1.append(selected_segment_1)
                    
    return selected_segments_1, selected_coords
...
def second_segment_selection_procedure(contour_corrected, selected_segments_1, dt, threshold_angle):
    ...
    for selected_segment in selected_segments_1:
        ...
        i_segment_t0 = selected_segment[16]
        
        # k, l = calculate_kl(V_nx, V_ny, x_A, y_A, x_piv, y_piv, dx, dy)
        ...
        segment_A, segment_B = (x_A, y_A), (x_B, y_B)
        
        V_n_direction = np.array([V_nx, V_ny])
        V_n_magnitude = np.linalg.norm(V_n_direction)
        V_n_extension = 100*V_n_direction/V_n_magnitude
        V_n_coord0 = (x_piv, y_piv)
        V_n_coord1 = (V_n_coord0[0] + V_n_extension[0], V_n_coord0[1] +  V_n_extension[1])
        
        intersect, (x_intersect_ref, y_intersect_ref) = check_intersection_normal_vector_and_segment(segment_A, segment_B, V_n_coord0, V_n_coord1)
        ...
        if intersect:
            ...
            for i_segment in range(0, len(contour_x) - 1):
                ...
                k, l = calculate_kl(x_piv, y_piv, V_nx, V_ny, x_A, y_A, dx, dy)
                ...
                if distance_A <= L and distance_B <= L:
                    ...
                    if (flame_front_displacement < flame_front_displacement_ref and (flame_front_displacement < 2*L)):
                        ...
                        if (np.abs(segment_angle - segment_angle_ref)) < np.deg2rad(threshold_angle):
                            ...
                            selected_segment_2 = [selected_segment, x_intersect_ref, y_intersect_ref, x_intersect, y_intersect, i_segment, contour_corrected, flame_front_displacement, S_a]
                            ...
                            if counter < 1:
                                ...
                                # print(k, l, x_piv, y_piv, np.sqrt(V_nx**2 + V_ny**2), V_nx, V_ny)
...
def plot_velocity_vectors(fig, ax, x, y, u, v, scale_vector):
    ax.quiver(x, y, u, v, angles='xy', scale_units='xy', scale=scale_vector, ls='solid', fc='k', ec='k')
...
def plot_local_flame_speed(ax, final_segment, color):
    ...
    ax.text(x_piv, y_piv, np.round(V_n, 3), c='y')
    ...
    ax.quiver(x_piv, y_piv, V_tx, V_ty, angles='xy', scale_units='xy', scale=scale_vector, ls='-', fc='g', ec='g', width=0.015, headwidth=4, headlength=6)
    ax.quiver(x_piv, y_piv, V_nx, V_ny, angles='xy', scale_units='xy', scale=scale_vector, ls='-', fc=color, ec=color, width=0.015, headwidth=4, headlength=6)
    # ax.quiver(x_piv, y_piv, V_x, V_y, angles='xy', scale_units='xy', scale=scale_vector, ls='-', fc='k', ec='k', width=0.015, headwidth=4, headlength=6)
